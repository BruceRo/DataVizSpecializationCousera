...
```{r,problem4}
boxplot(dat$faminc_new, main = "Bruce's Boxplot of Family Income", xlab= "Family Income")
```
...
sm_dat <- dat %>% filter(gender == 1 & (region == 1 | region == 2) & marstat == 1 & newsint == 1)
str(sm_dat)
```
...
dat2 <- dat %>% mutate(tax_scale = CC18_325d%%2 + CC18_325a%%2) #modular division converts 2(oppose) to 0 and leaves 1 as 1
dat2$tax_scale[1:20]
```
...
```{r problem10}
dat %>% 
  filter(faminc_new >= 5 & faminc_new < 11 & investor == 2) %>% 
...
```{r problem11}
kable(summarise(dat, Mean = mean(dat$faminc_new), Median = median(dat$faminc_new), "Std. Dev." = sd(dat$faminc_new)))
```
...
qplot(x = pid7, data = dat, geom = "histogram", xlab = "Seven Point Party ID", ylab = "Count")
```